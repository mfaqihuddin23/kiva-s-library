...
function randRange(min, max): number {
  ...
  function mapRange(value, low1, high1, low2, high2): number {
  ...
  const getPointerInput = (callback: function, element: HTMLElement = document, delay: number = 600) => {
  ...
  class StarField {
    constructor(howManyStars: number, canvas: HTMLElement, depth: number = 2, UIFadeDelay = 1) {
      this.canvas = canvas;
  ...
  function setup() {
    ...
    const howManyStars: number = 1000;
    if (IS_MOBILE) howManyStars = 500;
    
    let starfield: StarField = new StarField(howManyStars, canvas);
